...
static int addNode(LinkedList *this, int nodeIndex, void *pElement) {
	...
	if (this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this)) //validacion
		...
		pNewNode = (Node*) malloc(sizeof(Node)); //pido memoria para el nuevonodo

		if (pNewNode != NULL) //si no es nulo
		{
			pNewNode->pElement = pElement; //le cargo el elemento q me mandaron
			pNewNode->pNextNode = NULL; //todavia no se si hay un nodo posterior, entonces le pongo null

			if (nodeIndex == 0) //si el index es cero, entonces es el primer nodo
					{

				pNewNode->pNextNode = this->pFirstNode; //si el index es cero entonces es el first node
				this->pFirstNode = pNewNode; //llamo a la ll y le pido el primernodo

			} else {

				pPreviousNode = getNode(this, nodeIndex - 1); //pido el nodo, pero el indice es -1 porq es el anterior
				pNewNode->pNextNode = pPreviousNode->pNextNode; //la direccion de memoria del nuevo nodo es la del previo
				pPreviousNode->pNextNode = pNewNode; //y ahora el previo tiene la dir del nuevo o proximo
			}
